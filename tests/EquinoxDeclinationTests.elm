module EquinoxDeclinationTests exposing (tests)

import CommonModel exposing (InputData)
import Expect
import SunHelper exposing (sunDeclination)
import GregorJDN exposing (jdateGr, weekday, yearLength)
...
-- for times 1:01 to 1:22
-- London 2022-09-23 autumn equinox at 01:04:27


london =
    InputData "2022" "9" "23" "1" "4" "27" "51.5" "-0.13" "1"


mods1 =
    List.map (\mn -> { london | hour = "0", minute = String.fromInt mn }) minutes1


mods2 =
    List.map (\mn -> { london | hour = "1", minute = String.fromInt mn }) minutes2


models =
    List.append mods1 mods2


equinoxDeclinations =
    List.map sunDeclination models
...
testPairs =
    Zip.zip equinoxDeclinations declinationsNOAA
