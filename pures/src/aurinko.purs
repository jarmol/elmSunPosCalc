...
main  =
  ...
  log $ "Mean anomaly normalized  "
     <> toStringWith (fixed 6) normAnomal
  log $  "Mean longitude of Sun "
     <> toStringWith (fixed 6) meanLongitude <> "°"
  log $  "Mean longitude normalized "
     <> toStringWith (fixed 6) normLongit <> "°"
  log $  "Earth orbit eccentrity " <> orbitEccentrity
  log $ "Sun eq. of center " <> sunEquationCenter
  log $ "Sun true longitude "
    <> toStringWith (fixed 5) sunTrueLongitudeNormal <> "°"
  log $ "Sun apparent longitude "
    <> toStringWith (fixed 5) apparentLongitude <> "°"
  log $ "Apparent longitude normalized "
    <> toStringWith (fixed 5) apparLongitNormal <> "°"
  log $ "Mean oblique ecliptic "
    <> toStringWith (fixed 5) meanObliqueEcliptic <> "°"
