...
//  cityList += '<li>' + cities[i].city + "</li>";
    cityList += (i + 1) + '. ' + cities[i].city + ', ';
    if ((i + 1) % 7 == 0) {cityList += '<br>'};
}
...
const form = document.getElementById("cityForm");
    form.addEventListener("submit", (evls) => {
      evls.preventDefault(); // don't reload/submit the page

let text = "";
let cityNumber = document.getElementById("cityForm").cityNumberInput.value.trim();
let i = cityNumber - 1;
text += "<div>" + cities[i].city + ", latitude " + cities[i].latitude + "°, longitude " + cities[i].longitude +
   "°, timezone " + cities[i].timezone;

let local_time = new Date();
let local_time_string = local_time.toString();

text += "<br>UTC date and time : " + local_time.toUTCString().slice(5,26);

let dts = true; // Daylight saving

let otherCity = cities[i].city;
if (otherCity == 'Tokyo') {dts = false}; // No DTS in Japan!
let otherOffset = cities[i].timezone;
if ((dts) && (cities[i].latitude > 0)) {otherOffset += 1}; // Add DTS hour to timezone offset in northern world
if ((!dts) && (cities[i].latitude < 0)) {otherOffset += 1}; // Add DTS hour to timezone offset in southern world

let otherTime = calcTime(otherOffset);
text += "<br>" + otherCity + " local time: " + otherTime + 'h';
text += "<br>Your local time now: " + local_time_string.slice(3, 58);
...
let geomMeanLong =
      (280.46646 + (jCent * (36000.76983 + jCent * 0.0003032))) % 360.0;

let geomMeanAnom = 
      (357.52911 + jCent * (35999.05029 - 0.0001537 * jCent)) % 360.0;
...
let sunEqOfCtr =
      singA * (1.914602 - jCent * (0.004817 + 0.000014 * jCent))
        + singA2 * (0.019993 - 0.000101 * jCent)
        + singA3 * 0.000289;
      
console.log("JD = ", JD, "JDN = ", JDN, "jCent = ", jCent);
...
text += "<p>JDN " + JDN + ", JD " + JD.toFixed(6) + ", jCent " + jCent.toFixed(6) + "</p>";
...
let timezone = cities[i].timezone;
let dst_hour = 0;
if (dts) {dst_hour = 1;}
let tzOffset = timezone + dst_hour;  // Adjust for daylight saving time
let haSunrise =
         deg(Math.acos(Math.cos(rad(90.833)) / (Math.cos(rad(latitude)) * Math.cos(rad(sunDeclin)))
...
text += "<p>Solar Noon " + hours + ":" + minutes + ":" + seconds + "</p>"; 
...
if (sunset_time < sunrise_time) {dayLength += 1440;}
...
text += "<p>" + sunriseText + "</p>"
      + "<p>" + sunsetText + "</p>"
      + "<p>" + dayLengthText + "</p></div>"
